...
public class DormitoryController {
    ...
    @Operation(summary = "缴纳电费", security = {@SecurityRequirement(name = "Authorization")})
    public Result<Boolean> payElectricity(@Parameter(description = "宿舍id") @PathVariable Long id,
        ...
        boolean b = dormitoryService.update(Wrappers.<Dormitory>lambdaUpdate()
                .setSql("electricity = electricity + %f".formatted(money * payElectricity))
                .eq(Dormitory::getId, id));
        if (!b)
    ...
    /**
     * 获取宿舍的用户信息
     * 连表查询先查询所有的用户id，在依据用户id查询用户信息，
     * @param id 宿舍id
     * @return
     */
    @GetMapping("/user/{id}")
    @Operation(summary = "获取宿舍的用户信息", security = {@SecurityRequirement(name = "Authorization")})
    public Result<SysUser> getUser(@Parameter(description = "宿舍id") @PathVariable Long id) {
        return Result.success(dormitoryService.getUserByDormitoryId(id));
    }

    @GetMapping("/user")
    @Operation(summary = "分页查询用户和宿舍信息", security = {@SecurityRequirement(name = "Authorization")})
    public PageResult<UserDormitoryVo> getUserPage(@ParameterObject DormitoryPageQuery queryParams) {
        return PageResult.success(dormitoryService.getUserPage(queryParams));
    }
}